# 🎮 Sadeepa Piyumal Ranasinghe
**Unreal Engine Game Developer | SLIIT Undergraduate**

📧 Email: sadeepapiyumal530@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/sadeepa-piyumal-ranasinghe-198728287  
💻 GitHub: https://github.com/sadeepapiyumal  

---

## 👋 About Me
Final-year undergraduate at the Sri Lanka Institute of Information Technology (SLIIT), specializing in **Interactive Media**, with hands-on experience in **Unreal Engine 5.5**, **C++ gameplay programming**, and **Blueprint-based systems**.  
Currently seeking a **internship** or **junior Unreal Engine role**.

---

## 🛠 Technical Skills
- Unreal Engine 5.6
- C++ (Gameplay Programming – Beginner)
- Blueprint Visual Scripting
- AI: Behavior Trees, NavMesh
- UI: Unreal Motion Graphics (UMG)
- Gameplay Systems: Shooting, Health & Damage, Interaction
- Version Control: Git & GitHub

---

## 🚀 Projects

### 🔹 Shooter Sam (UE 5.6)
**Third-Person Shooter | Solo Project**

A third-person shooter game focusing on combat mechanics and intelligent enemy behavior.

**Key Features:**
- Enemy AI implemented using **Behavior Trees**
- Shooting mechanics with health and damage systems
- HUD displaying player health and targeting crosshair
- Gun system with animations, sound effects, and particle effects
- Core gameplay systems developed using **C++ and Blueprints**

🎥 Gameplay Video: https://drive.google.com/drive/folders/1PLCjmYknxosa7RfjZjh6X6BP8vmNBwfk?usp=sharing

📁 Source Code: https://github.com/sadeepapiyumal/Shooter_Sam_UE5.6


---

### 🔹 Dungeon Escape (UE 5.6)
**First-Person Puzzle Game | Solo Project**

A puzzle-based first-person game where players interact with the environment to escape a dungeon.

**Key Features:**
- Pressure plate mechanics using mover and trigger components
- Inventory system for collecting and using items
- Puzzle-driven level progression
- Atmospheric lighting using skylights and spotlights
- Gameplay logic implemented in **C++**

🎥 Gameplay Video: https://drive.google.com/drive/folders/14UodE9D5d-69jAbU9F-dviWvUsfr7yZr?usp=drive_link

📁 Source Code: https://github.com/sadeepapiyumal/Dungeon-Escape-UE5.6


---

### 🔹 Warehouse Wreckage (UE 5.6)
**Physics-Based Destruction Game | Solo Project**

A physics-driven game focused on launching projectiles to destroy structures.

**Key Features:**
- Physics-enabled objects and destruction mechanics
- Dynamic projectile spawning and firing system
- Aiming mechanics and ammunition management
- On-screen UI displaying remaining ammo
- Creative level building using Blueprint extensions

🎥 Gameplay Video: https://drive.google.com/drive/folders/1valOak7BARo75EF7u6IIZ0TPxsNbXfw-?usp=sharing

📁 Source Code: https://github.com/sadeepapiyumal/Warehouse-Wreckage-UE5.6

---

## 📄 CV
📥 [Download CV (PDF)](https://github.com/sadeepapiyumal/Unreal-Engine-Portfolio/blob/main/Sadeepa_Piyumal_Ranasinghe_Unreal_Developer_CV.pdf)

---

## 📫 Contact
I am open to internship and junior opportunities in Unreal Engine game development.  
Feel free to contact me via email or LinkedIn.
